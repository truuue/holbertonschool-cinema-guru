...
router.get("/favorite/", verifyToken, (req, res) => {
  User.findOne({ where: { id: req.userId }, include: { model: Title, as: "favorite" } })
    .then(user => {
...
router.get("/watchLater/", verifyToken, (req, res) => {
  User.findOne({ where: { id: req.userId }, include: { model: Title, as: "watchLater" } })
    .then(user => {
...
router.delete("/favorite/:imdbId", verifyToken, async (req, res) => {
  ...
  try {
    ...
    await (
      await UserFavorites.findOne({ where: { UserId: req.userId, TitleId: title.id } })
    ).destroy();
...
router.delete("/watchlater/:imdbId", verifyToken, async (req, res) => {
  ...
  try {
    ...
    const user = await User.findOne({ where: { id: req.userId } });
    await (
      await UserWatchLater.findOne({ where: { UserId: req.userId, TitleId: title.id } })
    ).destroy();
    const userActivity = await UserActivity.create({
      userId: user.id,
      TitleId: title.id,
